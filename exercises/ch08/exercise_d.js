...
const validateName = ({ name }) => ((name.length < 3)
  ? left('Name must be more than 3 characters')
  : Either.of());

const showWelcomeAndSave = compose(map(showWelcome), save);

// register :: User -> IO String
const register = compose(either(IO.of, showWelcomeAndSave), validateUser(validateName));
